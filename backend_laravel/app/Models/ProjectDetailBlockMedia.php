...
class ProjectDetailBlockMedia extends Model
{
    use HasFactory;

    protected $fillable = [
        'project_detail_block_id',
        ...
        'order', // Добавлено поле order для сортировки
    ];

    // Связь с блоком контента
    public function block()
    {
        return $this->belongsTo(ProjectDetailBlock::class, 'project_detail_block_id');
    }
    ...
    // Статический метод для получения данных в формате галереи
    public static function getGalleryData($blockId)
    {
        $mediaItems = static::where('project_detail_block_id', $blockId)
            ->orderBy('order') // Добавлена сортировка по order
            ->get()
            ->groupBy('group_id');
